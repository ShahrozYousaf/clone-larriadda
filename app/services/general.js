...
export class General {
    config;
    latestmenu_loaded = false;
    walkingcustomerapp_signature;
    cartitems_list = [];
    categories_list = [];
    items_list = [];
    branch_result = undefined;
    constructor(config) {
        this.config = config;
    }
    setup() {
        ...
        if (cartitems_list) {
            this.cartitems_list = this.parseStorageList(cartitems_list);
        }
        let categories_list = localStorage.getItem('categories_list');
        if (categories_list) {
            this.categories_list = this.parseStorageList(categories_list);
        }
        let items_list = localStorage.getItem('items_list');
        if (items_list) {
            this.items_list = this.parseStorageList(items_list);
        }
        let branch_result = localStorage.getItem('branch_result');
        if (branch_result) {
            this.branch_result = JSON.parse(branch_result);
        }
    }
    parseStorageList(value) {
        try {
    ...
    call_itemfinalprice(rate, drate) {
    ...
    updateReponse(response) {
        ...
        if (response['items_list']) {
            ...
            let items_list = [];
            let categories_list = [];
            dbitems_list.forEach((item, index) => {
                ...
                let findalready = categories_list.find((subcategory) => subcategory.item_gcategory === item_gcategory);
                if (!findalready) {
                    let jsonRecord = {};
                ...
                if (item_originalprice > item_displayprice) {
                    check_itemdiscountfound = 1;
                }
    ...
    static ɵfac = function General_Factory(__ngFactoryType__) { return new (__ngFactoryType__ || General)(i0.ɵɵinject(APP_CONFIG)); };
    static ɵprov = /*@__PURE__*/ i0.ɵɵdefineInjectable({ token: General, factory: General.ɵfac, providedIn: 'root' });
}
(() => { (typeof ngDevMode === "undefined" || ngDevMode) && i0.ɵsetClassMetadata(General, [{
        type: Injectable,
        args: [{
                providedIn: 'root',
            }]
